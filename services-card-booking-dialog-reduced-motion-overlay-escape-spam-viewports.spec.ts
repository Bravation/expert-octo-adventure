...
const PROVIDER_ID = "00000000-0000-0000-0000-000000000996";

const FAKE_PROVIDER = {
  id: PROVIDER_ID,
  full_name: "RM Overlay+Escape Provider",
  avatar_url: null,
  city: "Miami",
  state: "FL",
  average_rating: 4.5,
...
const FAKE_SERVICES = [
  {
    ...
    title: "RM Overlay Escape Plumbing",
    description: "Plumbing for RM overlay+escape test.",
    price: 145,
    category: "Plumbing",
  ...
  },
  {
    ...
    title: "RM Overlay Escape Lawn Care",
    description: "Lawn for RM overlay+escape test.",
    price: 75,
    category: "Lawn Care",
    provider_id: PROVIDER_ID,
    photo_url: null,
...
test.describe("Booking dialog — reduced motion: overlay click + Escape spam restores focus", () => {
  ...
  for (const vp of VIEWPORTS) {
    test(`Overlay click + Escape spam restores focus (${vp.name})`, async ({ page }) => {
      ...
      for (const title of ["RM Overlay Escape Plumbing", "RM Overlay Escape Lawn Care"]) {
        ...
        // Radix renders the overlay as a sibling with [data-state="open"]
        // outside [role="dialog"]; click at viewport corner where overlay
        // is guaranteed to receive the click.
        const overlay = page
        ...
        const overlayClick = (async () => {
          if ((await overlay.count()) > 0) {
            ...
            // Fallback: click corner of viewport (outside dialog content)
