...
/**
 * Playwright test:
 * Verify the booking dialog's focusable order matches the expected visual
 * tab sequence — i.e., pressing Tab visits each focusable element in DOM
 * order, with element identity matching between the live focused element
 * and the precomputed focusable list. Runs across mobile, tablet, and
 * desktop viewports.
 */
...
const FAKE_PROVIDER = {
  id: PROVIDER_ID,
  full_name: "Tab Order Sequence Provider",
  avatar_url: null,
  city: "Miami",
  state: "FL",
  average_rating: 4.5,
...
const FAKE_SERVICES = [
  {
    ...
    title: "Tab Order Sequence Plumbing",
    description: "Plumbing for tab-order sequence test.",
    price: 145,
    category: "Plumbing",
  ...
  },
  {
    ...
    title: "Tab Order Sequence Lawn Care",
    description: "Lawn for tab-order sequence test.",
    price: 75,
    category: "Lawn Care",
    provider_id: PROVIDER_ID,
    photo_url: null,
...
const FOCUSABLE_MARK_ATTR = "data-tab-order-id";
...
test.describe("Booking dialog: Tab order matches expected visual sequence", () => {
  ...
  for (const vp of VIEWPORTS) {
    test(`Tab visits focusables in DOM order (${vp.name})`, async ({ page }) => {
      await page.setViewportSize({ width: vp.width, height: vp.height });

      for (const title of [
        "Tab Order Sequence Plumbing",
        "Tab Order Sequence Lawn Care",
      ]) {
        ...
        let startIdx = expectedIds.indexOf(currentId!);
        expect(startIdx, `[${vp.name}] focused element should be in stamped list`).toBeGreaterThanOrEqual(0);

        // Walk one full cycle of Tab presses and assert identity at each step.
        const observed: string[] = [currentId!];
        for (let i = 0; i < expectedIds.length; i++) {
          await page.keyboard.press("Tab");
          const id = await readFocusedId(page, FOCUSABLE_MARK_ATTR);
          const expected = expectedIds[(startIdx + i + 1) % expectedIds.length];
          expect(
            id,
            `[${vp.name}] Tab #${i + 1} should focus expected element "${expected}" but got "${id}" ("${title}")`,
          ).toBe(expected);
          observed.push(id!);
        }

        // After a full cycle (length presses from start), focus should return
        // to the starting element.
        expect(
          observed[observed.length - 1],
          `[${vp.name}] full Tab cycle should return to start ("${title}")`,
        ).toBe(currentId);
