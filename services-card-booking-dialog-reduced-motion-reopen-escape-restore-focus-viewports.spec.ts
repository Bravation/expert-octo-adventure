...
/**
 * Reduced-motion mirror of the reopen-escape focus restoration test.
 *
 * Verifies that under prefers-reduced-motion: reduce, after closing the
 * booking dialog with Escape and REOPENING it from the same originating
 * Services control, a second Escape still restores focus to that same
...
const PROVIDER_ID = "00000000-0000-0000-0000-0000000000b4";

const FAKE_PROVIDER = {
  id: PROVIDER_ID,
  full_name: "RM Reopen Escape Provider",
  avatar_url: null,
  city: "Charlotte",
  state: "NC",
  average_rating: 4.5,
...
const FAKE_SERVICES = [
  {
    ...
    title: "RM Reopen Plumbing",
    description: "Plumbing for reduced-motion reopen-escape test.",
    price: 168,
    category: "Plumbing",
  ...
  },
  {
    ...
    title: "RM Reopen Lawn Care",
    description: "Lawn for reduced-motion reopen-escape test.",
    price: 91,
    category: "Lawn Care",
    provider_id: PROVIDER_ID,
    photo_url: null,
...
test.describe("Booking dialog — reopen after Escape still restores focus (reduced motion)", () => {
  ...
  for (const vp of VIEWPORTS) {
    test(`Escape → reopen → Escape restores focus to same control (${vp.name})`, async ({
      ...
      for (const title of ["RM Reopen Plumbing", "RM Reopen Lawn Care"]) {
