 ...
 * While the booking dialog is open, interleave Tab navigation inside the
 * dialog with repeated Escape presses. Verify focus stays trapped during
 * the Tab phase, the dialog ultimately closes, no errors are thrown, and
 * focus is restored to the originating Services control.
 * Runs across mobile, tablet, and desktop viewports.
 */

const PROVIDER_ID = "00000000-0000-0000-0000-000000000995";

const FAKE_PROVIDER = {
  id: PROVIDER_ID,
  full_name: "RM Tab+Escape Provider",
  avatar_url: null,
  city: "Miami",
  state: "FL",
  average_rating: 4.5,
...
const FAKE_SERVICES = [
  {
    ...
    title: "RM Tab Escape Plumbing",
    description: "Plumbing for RM tab+escape test.",
    price: 145,
    category: "Plumbing",
  ...
  },
  {
    ...
    title: "RM Tab Escape Lawn Care",
    description: "Lawn for RM tab+escape test.",
    price: 75,
    category: "Lawn Care",
    provider_id: PROVIDER_ID,
    photo_url: null,
...
async function dialogIsOpen(page: Page): Promise<boolean> {
  return page.evaluate(() => {
    const dialog = document.querySelector('[role="dialog"]');
    return !!dialog && dialog.getAttribute("data-state") === "open";
  });
}

test.describe("Booking dialog — reduced motion: interleaved Tab + Escape restores focus", () => {
  ...
  for (const vp of VIEWPORTS) {
    test(`Tab held + Escape restores focus to origin (${vp.name})`, async ({ page }) => {
      ...
      for (const title of ["RM Tab Escape Plumbing", "RM Tab Escape Lawn Care"]) {
        ...
        // Interleave Tab navigation with Escape presses
        for (let round = 0; round < ROUNDS; round++) {
          if (!(await dialogIsOpen(page))) break;

          for (let t = 0; t < TABS_PER_ROUND; t++) {
            await page.keyboard.press("Tab");
            if (!(await dialogIsOpen(page))) break;
            expect(
              await focusInsideDialog(page),
              `[${vp.name}] focus escaped during Tab phase round ${round + 1} ("${title}")`,
            ).toBe(true);
          }
        ...
        // Drain any remaining open state with extra Escapes
        for (let i = 0; i < 3; i++) {
          await page.keyboard.press("Escape");
        }
        ...
        expect(
          after,
          `[${vp.name}] focus should restore after Tab+Escape interleave ("${title}")`,
        ).not.toBeNull();
        ...
        expect(
          errors,
          `[${vp.name}] no page or console errors during Tab+Escape interleave ("${title}")`,
        ).toEqual([]);
      }
    });
  }
