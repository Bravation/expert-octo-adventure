 ...
 * While the booking dialog is open, run a rapid Tab/Shift+Tab burst, verify
 * focus never escapes the dialog during the burst, then press Escape and
 * verify the dialog closes and focus returns to the originating Services
 * control. Runs across mobile, tablet, and desktop viewports.
 */

const PROVIDER_ID = "00000000-0000-0000-0000-000000000993";

const FAKE_PROVIDER = {
  id: PROVIDER_ID,
  full_name: "RM Escape Spam Provider",
  avatar_url: null,
  city: "Miami",
  state: "FL",
  average_rating: 4.5,
...
const FAKE_SERVICES = [
  {
    ...
    title: "RM Escape Spam Plumbing",
    description: "Plumbing for RM escape-spam test.",
    price: 145,
    category: "Plumbing",
  ...
  },
  {
    ...
    title: "RM Escape Spam Lawn Care",
    description: "Lawn for RM escape-spam test.",
    price: 75,
    category: "Lawn Care",
    provider_id: PROVIDER_ID,
    photo_url: null,
...
test.describe("Booking dialog — reduced motion: Escape after Tab/Shift+Tab burst restores focus and never escapes dialog", () => {
  ...
  for (const vp of VIEWPORTS) {
    test(`Tab/Shift+Tab burst stays trapped, Escape restores focus (${vp.name})`, async ({
      page,
    }) => {
      await page.setViewportSize({ width: vp.width, height: vp.height });

      for (const title of ["RM Escape Spam Plumbing", "RM Escape Spam Lawn Care"]) {
        ...
        for (let i = 0; i < TAB_BURST; i++) {
          await page.keyboard.press("Tab");
          if ((i + 1) % 10 === 0) {
            expect(
              await focusInsideDialog(page),
              `[${vp.name}] focus escaped during Tab burst at ${i + 1} ("${title}")`,
            ).toBe(true);
          }
        }
        for (let i = 0; i < SHIFT_TAB_BURST; i++) {
          await page.keyboard.press("Shift+Tab");
          if ((i + 1) % 10 === 0) {
            expect(
              await focusInsideDialog(page),
              `[${vp.name}] focus escaped during Shift+Tab burst at ${i + 1} ("${title}")`,
            ).toBe(true);
          }
        }
        ...
        expect(
          await focusInsideDialog(page),
          `[${vp.name}] focus must remain in dialog before Escape ("${title}")`,
        ).toBe(true);
        ...
        await expect(async () => {
          ...
          expect(
            after,
            `[${vp.name}] focus should restore after Escape ("${title}")`,
          ).not.toBeNull();
